...
async function crearProducto() {
    const data = { 
        ...
        activo: document.getElementById('n-stock').value === "true" // Lee el selector de stock
    };
...
function editar(p) {
    ...
    document.getElementById('e-stock').value = p.activo ? "true" : "false"; // Marca el stock actual
...
async function cargarPedidos() {
    ...
    if (ords.length === 0) { tbody.innerHTML = '<tr><td colspan="4" style="text-align:center; color:var(--text-muted);">No hay órdenes.</td></tr>'; return; }
    tbody.innerHTML = ords.map(o => `<tr><td style="font-weight:bold; color:var(--accent);">#${o.id}</td><td><strong style="color:var(--text);">${o.cliente_nombre}</strong><br><span style="color:var(--text-muted); font-size:0.9rem;">📞 ${o.cliente_telefono}<br>📍 ${o.cliente_direccion || 'N/A'}</span></td><td><strong style="font-size:1.1rem; color:var(--text);">${o.total}</strong><br><small style="background:rgba(255,255,255,0.1); padding:3px 6px; border-radius:4px; color:var(--text);">${o.metodo_pago ? o.metodo_pago.toUpperCase() : 'N/A'}</small></td><td><select class="form-input" onchange="cambiarEstado(${o.id}, this.value)" style="padding:6px; font-weight:bold; cursor:pointer; width:auto; margin:0;"><option value="Pendiente" ${o.estado === 'Pendiente' ? 'selected' : ''}>Pendiente ⏱️</option><option value="Pagado" ${o.estado === 'Pagado' ? 'selected' : ''}>Pagado 💰</option><option value="Enviado" ${o.estado === 'Enviado' ? 'selected' : ''}>Enviado 📦</option></select></td></tr>`).join('');
}
...
async function guardarConfiguracion() {
    ...
    await fetch('/api/admin/configuracion', { method: 'PUT', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ cotizacion: cotizacion, banner_url: bannerUrl }) });
