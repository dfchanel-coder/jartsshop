...
function showTab(id, element) {
    ...
    // Cargar datos según la pestaña elegida
...
async function crearProducto() {
    ...
    if (!data.nombre || !data.precio || !data.imagen_url) {
...
async function cargarInventario() {
    ...
    if (prods.length === 0) {
        tbody.innerHTML = '<tr><td colspan="3" style="text-align:center;">El inventario está vacío.</td></tr>';
        return;
    }
    ...
    tbody.innerHTML = prods.map(p => `
        <tr>
            <td style="width:60px;">
                <img src="${p.imagen_url}" style="width:50px; height:50px; object-fit:cover; border-radius:5px; border:1px solid #ddd;">
            </td>
            <td>
                <strong style="font-size:1.1rem;">${p.nombre}</strong><br>
                <span style="color:#666;">$${p.precio} | Categ: ${p.categoria}</span>
            </td>
            <td style="width:150px;">
                <button class="btn-accion btn-editar" onclick='editar(${JSON.stringify(p)})'>Editar</button>
                <button class="btn-accion btn-borrar" onclick='borrar(${p.id})'>Borrar</button>
            </td>
        </tr>
    `).join('');
}
...
async function borrar(id) { 
    if (confirm('¿Estás seguro de eliminar este perfume de la tienda?')) { 
...
async function cargarResenasAdmin() {
    ...
    if (resenas.length === 0) {
        tbody.innerHTML = '<tr><td colspan="5" style="text-align:center;">No hay reseñas aún en la tienda.</td></tr>';
        return;
    }

    tbody.innerHTML = resenas.map(r => `
        <tr>
            <td style="color:var(--accent); font-weight:bold;">${r.perfume_nombre}</td>
            <td><strong>${r.nombre}</strong></td>
            <td style="max-width:300px; font-style:italic; color:#555;">"${r.comentario}"</td>
            <td style="font-size:1.1rem;">${r.estrellas}</td>
            <td>
                <button onclick="borrarResena(${r.id})" class="btn-accion btn-borrar">Borrar</button>
            </td>
        </tr>
    `).join('');
}
...
async function borrarResena(id) {
    if (confirm('⚠️ Atención:\n\n¿Estás seguro de que deseas borrar este comentario? Esta acción no se puede deshacer y desaparecerá de la tienda pública inmediatamente.')) {
        await fetch(`/api/admin/resenas/${id}`, { method: 'DELETE' }); 
        cargarResenasAdmin(); // Refresca la tabla
    }
}

// Iniciar verificando si está logueado
check();